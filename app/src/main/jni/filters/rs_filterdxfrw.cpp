...
#include "../engine/rs_vector.h"
#include "../engine/rs_polyline.h"
#include "../engine/rs_pen.h"
#include "../debug/rs_debug.h"
#include "../libdxfrw/libdwgr.h"
#include "../engine/rs_block.h"


#include "../engine/rs_arc.h"
//#include "rs_circle.h"
...
//#include "rs_image.h"
//#include "rs_insert.h"
#include "../engine/rs_layer.h"
//#include "rs_leader.h"
#include "../engine/rs_line.h"
#include "../engine/rs_mtext.h"
//#include "rs_point.h"
//#include "rs_solid.h"
//#include "rs_spline.h"
 ...
 */
bool RS_FilterDXFRW::fileImport(RS_Graphic &g, const QString &file) {
...
//        return;
    ...
    circleList.emplace_back(v, data.radious);
//    setEntityAttributes(entity, &data);
//
//    currentContainer->addEntity(entity);
}
...
//                 data.endangle,
//                 false);
    RS_Vector v1(data.radious, data.staangle, data.endangle);
    arcList.emplace_back(v, v1);
...
//        return;
    ...
    if (fabs(ang2 - 2. * M_PI) < RS_TOLERANCE &&
        ...
        ang2 = 0.;
    ellipseList.emplace_back(v1, v2);
    RS_Vector v3(data.staparam, ang2);
    ellipseExList.emplace_back(v3, data.ratio);
...
//    if(verList.size()>2) return;
    lwLineList.emplace_back(verList);
    lwLineVertexNum.emplace_back(verList.size());


    //RS_Pen  pen=polyline->getPen();

//    RS2::LineType lineType = polyline->getPen().getLineType();
//    lineTypeList.emplace_back(lineType);
//    lineTypes.emplace_back(lineType);
//
...
//        return;
    ...
    lineTypeList.emplace_back(lineType);
    lineTypes.emplace_back(lineType);
//    polyline->getPen().getLineType();
//    polyline->getPen().getWidth();
...
//        verListLwLine.emplace_back(
//                std::make_pair(RS_Vector{v->basePoint.x, v->basePoint.y}, v->bulge));
    }
//    verListLwLineNum.emplace_back(verList.size());
...
//    double angle = data.angle * M_PI / 180.;
    RS_Vector ip = RS_Vector(data.basePoint.x, data.basePoint.y, data.height);
    std::string mctext = mtext.toStdString();
    mTextList.emplace_back(ip, mctext);
}
...
//    RS_DEBUG->print("RS_FilterDXFRW::addText");
    RS_Vector refPoint = RS_Vector(data.basePoint.x, data.basePoint.y, data.height);;
    RS_Vector secPoint = RS_Vector(data.secPoint.x, data.secPoint.y, data.height);;
    ...
    if (data.alignV != 0 || data.alignH != 0 || data.alignH == DRW_Text::HMiddle) {
        if (data.alignH != DRW_Text::HAligned && data.alignH != DRW_Text::HFit) {
            secPoint = RS_Vector(data.basePoint.x, data.basePoint.y, data.height);
            refPoint = RS_Vector(data.secPoint.x, data.secPoint.y, data.height);
        }
    }
...
//    }
    ...
    QString mtext = toNativeString(QString::fromUtf8(data.text.c_str()));
    std::string mctext = mtext.toStdString();
    mTextList.emplace_back(refPoint, mctext);
//    // use default style for the drawing:
//    if (sty.isEmpty()) {
//        // japanese, cyrillic:
//        if (codePage=="ANSI_932" || codePage=="ANSI_1251") {
...
QString RS_FilterDXFRW::toNativeString(const QString &data) {
        ...
        if (data.at(i).unicode() == 0x7B) { //is '{' ?
            if (data.at(i + 1).unicode() == 0x5c) { //and is "{\" ?
                ...
                if ((data.at(i + 2).unicode() == 0x66) || //is "\f" ?
                    ...
                    } while (tmp.startsWith("\\f") || tmp.startsWith("\\H") ||
                             tmp.startsWith("\\C"));
                    res.append(tmp);
                    i = j = pos;
                    ++j;
    ...
    // Tab:
    res = res.replace(QRegExp("\\^I"),
                      "    ");//RLZ: change 4 spaces for \t when mtext have support for tab
    // diameter:
    res = res.replace(QRegExp("%%[cC]"),
                      QChar(0x2300));//RLZ: Empty_set is 0x2205, diameter is 0x2300 need to add in all fonts
    // degree:
